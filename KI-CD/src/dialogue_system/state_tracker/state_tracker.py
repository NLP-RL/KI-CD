...
class StateTracker(object):
    def __init__(self, user, agent,parameter):
        self.user = user
        self.agent = agent
        self.symp = pickle.load(file=open(parameter["slot_set"], "rb"))
        self._init()
    ...
    def _init(self):
        self.turn = 0
        self.G = -1  ##Group
        self.GP = -1 ##Group Probabaility
        self.r = 0   ##repeatition
        #self.D = -1
        #self.DP = 0
        self.ProbableSym = [0]*len(self.symp)
        self.state = {
            "agent_action":None,
            "user_action":None,
            "turn":self.turn,
    ...
    def state_updateG(self,G,prob):
        self.G = G
        self.GP = prob
        self.state['StoG'] = G
        self.state['StoGP'] = prob
    def ProbableSymUpdate(self,D,DP,sym):
        self.probable_sym = sym
        self.D= D
        self.DP = DP
        self.state['ProbableSym'] = sym
        #self.state['Disease'] = D
        #self.state['DieaseProb'] = DP
    def repeatition(self):
        self.r = 1
