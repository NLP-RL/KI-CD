...
def state_to_representation_last(state, action_set, slot_set, disease_symptom, max_turn):
    ...
    probable_sym = state["probable_sym"]
    probable_grp = state["StoG"]
    probable_disease = state["Disease"]
    dis_prob = state["DieaseProb"]
    Rep = state["repeatition"]
    ...
    state_rep = np.hstack((current_slots_rep,probable_sym,probable_grp,probable_disease,dis_prob, user_action_rep, user_inform_slots_rep, user_request_slots_rep, agent_action_rep, agent_inform_slots_rep, agent_request_slots_rep, turn_rep))
    return state_rep
...
def reduced_state_to_representation_last(state, slot_set, parameter):
    ...
    print('Inside state : ',len(slot_set.keys()))
    c = current_slots_rep.reshape(1,len(slot_set.keys())*3)[0]
    state_rep = np.hstack([c,probable_grp,probable_sym,probable_disease,dis_prob,Rep, turn_rep]).flatten()
    #state_rep = current_slots_rep.reshape(1,len(slot_set.keys())*3)[0]

    return state_rep


# 3 * LEN(SLOT) + 118 + 1 + 1 +1 +1 +1
# 53 * 3 + 118 + 5
#
