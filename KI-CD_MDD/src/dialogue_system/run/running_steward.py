...
class RunningSteward(object):
    ...
    def __init__(self, parameter, checkpoint_path):
        ...
        self.best_result= {
            ...
            "average_reward": 0.0,
            "average_turn": 0,
            "average_repeated_action": 0,
            "average_match_rate2": 0.0,
            "ab_success_rate":0.0,
    ...
    def simulate(self, epoch_number, train_mode=False):
        ...
        for index in range(0, epoch_number,1):
            ...
            print('Best Success rate: ',self.best_result)
    ...
    def simulation_epoch(self, epoch_size, index):
        ...
        ##Changed 15th June21 for real_implicit_slots =0
        if (real_implicit_slots!=0):
        ...
        res = {"success_rate":success_rate, "average_reward": average_reward, "average_turn": average_turn,
               "average_wrong_disease":average_wrong_disease,'amr':match_rate,'amr2':match_rate2,"ab_success_rate":absolute_success_rate}
        ...
        with open("train_result.json", "a") as outfile:
            json.dump(R, outfile)
    ...
    def evaluate_model(self, dataset, index):
        ...
        absolute_success_rate = float("%.3f" % (float(absolute_success_count) / dataset_len))
        average_reward = float("%.3f" % (float(total_reward) / dataset_len))
        average_turn = float("%.3f" % (float(total_turns) / dataset_len))
        average_wrong_disease = float("%.3f" % (float(inform_wrong_disease_count) / dataset_len))
        match_rate2 = float("%.3f" % (float(num_of_true_slots) / float(real_implicit_slots)))
        if num_of_implicit_slots>0:
            #match rate表示agent所问道的症状当中是病人确实有的概率为多大。match rate2表示病人有多少比例的隐形症状被agent问出。
            match_rate=float("%.3f" %(float(num_of_true_slots)/float(num_of_implicit_slots)))
        ...
        res = {
            ...
            "DiseaseClassifier_Accuracy":0
        }
        R = {index:res}

        with open("TR.json", "a") as outfile:
            json.dump(R, outfile)
        ...
        if self.parameter.get("classifier_type")=="deep_learning" and self.parameter.get("disease_as_action") == False:
            acc = self.dialogue_manager.train_deep_learning_classifier(epochs=20)
        ...
        if self.parameter.get("use_all_labels") == True and self.parameter.get("agent_id").lower() in ["agenthrljoint", "agenthrljoint2"] and self.parameter.get('train_mode') == False:
            pickle.dump(self.dialogue_manager.disease_record, open('ResultData/disease_record.p', 'wb'))
            pickle.dump(self.dialogue_manager.lower_reward_by_group, open('ResultData/lower_reward_by_group.p', 'wb'))
            pickle.dump(self.dialogue_manager.master_index_by_group, open('ResultData/master_index_by_group.p', 'wb'))
            pickle.dump(self.dialogue_manager.symptom_by_group, open('ResultData/symptom_by_group.p', 'wb'))
