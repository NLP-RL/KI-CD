...
parser.add_argument("--epsilon", dest="epsilon", type=float, default=0.0, help="The greedy probability of DQN")
...
parser.add_argument("--save_performance_interval",dest="save_performance_interval", type=int, default=300, help="performance interval (int)")
...
parser.add_argument("--saved_model", dest="saved_model", type=str, default="./../../model/DQN/checkpoint/0619155315_agenthrljoint2_T28_ss100_lr0.0005_RFS84_RFF0_RFNCY0_RFIRS0_RFRA-44_RFRMT-66_mls0_gamma0.95_gammaW0.9_epsilon0.05_awd0_crs0_hwg0_wc0_var0_sdai0_wfrs44_dtft0_ird0_ubc0.985_lbc1e-10_data_RID0/")
parser.add_argument("--save_dialogue", dest="save_dialogue", type=boolean_string, default=True, help="Save the dialogue? [True, False]")
parser.add_argument("--dialogue_file", dest="dialogue_file", type=str, default="dialogue_file_test.txt", help="the file that used to save dialogue content.")
...
#parser.add_argument("--multi_GPUs", dest="multi_GPUs", type=boolean_string, default=True,help="The id of GPU on the running machine.")
...
parser.add_argument("--disease_as_action", dest="disease_as_action", type=boolean_string, default=False, help="Error probability when applying noisy channel?")
...
##PreTRained weight
parser.add_argument("--pre_train", dest="pre_train", type=boolean_string, default=True, help="using the disease tag for inform disease ? {True, False}")
...
def run(parameter):
    ...
    if train_mode is True: # Train
        ...
        for index in range(simulate_epoch_number):
            ...
            out[index] = res
            s_r += res['success_rate']
            m_r += res['average_match_rate']
            r_a += res['average_repeated_action']
            m_r_2 += res['average_match_rate2']
            r += res['average_reward']
            t += res['average_turn']
            acc += res['DiseaseClassifier_Accuracy']

            SR = SR + [res['success_rate']]
            MR = MR + [res['average_match_rate']]
            MR2 = MR2 + [res['average_match_rate2']]
            T =T + [res['average_turn']]
        ...
        print(t)
        print(acc)
        print('standard error for success rate : ', sem(SR))
        print('standard error for match rate : ', sem(MR))
        print('standard error for match rate 2: ', sem(MR2))
        print('standard error for turn : ', sem(T))
        print('standard error for DC : ', sem(DC))
...
if __name__ == "__main__":
    ...
    checkpoint_path = "src/model/DQN/checkpoint"
    # Flat-DQN, ss1
    # agent_id = 'AgentDQN'
    ...
    for date_time in date_time_list:
        ...
        for epoch_index in sorted(model_name_dict.keys()):
            ...
            print('Model Nameee : ',model_name)
            params['saved_model'] = os.path.join(model_path,model_name)
            print('Selected :',params['saved_model'])
