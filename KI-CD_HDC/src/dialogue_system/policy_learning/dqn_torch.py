...
class DQNModel2(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, parameter):
        ...
        # one layer.
        if parameter.get("initial_symptom"):
            self.policy_layer = torch.nn.Sequential(
                torch.nn.Linear(input_size, hidden_size, bias=True),
...
class DQN(object):
    def __init__(self, input_size, hidden_size, output_size, parameter, named_tuple=('state', 'agent_action', 'reward', 'next_state', 'episode_over')):
        ...
        if self.params["is_relational_dqn"] is False:
            if self.params.get("dqn_type") == "DuelingDQN":
                ...
                self.target_net = DuelingDQN(input_size, hidden_size, output_size, parameter).to(self.device)
            else:
    ...
    def restore_model(self, saved_model):
        ...
        if torch.cuda.is_available() is False:
            ...
            map_location = None
        #self.current_net.load_state_dict(torch.load(saved_model,map_location=map_location), strict = False)
        #self.target_net.load_state_dict(self.current_net.state_dict())
        model_old = torch.load(saved_model, map_location=map_location)
        ...
        # load params
        self.current_net.load_state_dict(new_state_dict)
        self.target_net.load_state_dict(self.current_net.state_dict())
...
class DQN2(object):
    ...
    #
    #     self.current_net.load_state_dict(torch.load(saved_model,map_location=map_location), strict = False)
    #     self.target_net.load_state_dict(self.current_net.state_dict(), strict = False)
    #     ##for Training
    #
    #     # model_old = torch.load(saved_model, map_location=map_location)
    #     # new_state_dict = OrderedDict()
